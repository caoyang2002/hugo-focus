...
function updateScrollMetrics() {
  ...
  scrollState.totalScroll =
    document.body.scrollHeight - scrollState.windowHeight;
}
...
function getScrollProgress() {
  ...
  scrollState.scrollProgress = Math.max(
    0,
    Math.min(1, scrollState.scrollTop / scrollState.totalScroll),
  );
...
function applyParallax() {
  ...
  // 背景层反向偏移
  if (layers.deepBg) {
    layers.deepBg.style.transform = `translate3d(0, ${-baseOffset * PARALLAX.deepBg}px, var(--z-deep-bg))`;
  }

  if (layers.grid) {
    layers.grid.style.transform = `translate3d(0, ${-baseOffset * PARALLAX.grid}px, var(--z-grid))`;
  }

  if (layers.line) {
    layers.line.style.transform = `translate3d(0, ${-baseOffset * PARALLAX.line}px, var(--z-line))`;
  }

  // 导航层保持固定，不应用任何transform
  if (layers.nav) {
    layers.nav.style.transform = "none";
  }
}
...
function handleWorksDisplay(progress) {
  ...
  // 阶段3：0.4 ~ 0.7 → 显示3D作品项
    ...
    progress > THRESHOLD.worksFull &&
    progress <= THRESHOLD.contactStart
  ) {
    updateNavigation("works");
  ...
  // 阶段4：0.7 ~ 0.9 → 作品项渐隐
    ...
    progress > THRESHOLD.contactStart &&
    progress <= THRESHOLD.contactFull
  ) {
